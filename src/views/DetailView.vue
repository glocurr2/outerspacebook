    ...
    <div class="book-detail">
      <div v-for="item in singlebook" :key="item.id" class="display-book">
        ...
        <div class="flex-container">
          <div>
            <img
              v-bind:src="SITE_URL + '/images/' + item.image"
          ...
          <div v-if="item.author_image" class="author-image">
            <img
              v-bind:src="SITE_URL + '/images/' + item.author_image"
...
let singlebook = {};
if (storeBooks.bookdata) {
  singlebook = allbooks.filter((item) => item.id == bookroute);
} else {
  router.push('/');
}
